...
namespace{
    ...
    constexpr float CURRENT_MEASURE_COEFF = 20.0;
...
void Measurement::readLevel(void){
    ...
    uint32_t iout = Measurement::read_current(0); // [micro Amp]

    // calc L-He level from the mesurement
    float ratio =1.0;
    // 電流が計測されていない場合[1mA以下]   0%  にする。
    if (iout != 0){
        ratio = ((float)Measurement::read_voltage(0,true)/(float)iout) / sensor_resistance ;
...
/*!
 * @brief オフセット電圧を測定する
 * @returns 計測した電圧    [LSB],[microVolt]
 * @param adc_channel 0:0-1 , それ以外:2-3
 * @param mode ReturnValue = true:[LSB] false:[microVolt]
 */
uint32_t Measurement::read_voltage(const uint16_t adc_channel, const boolean mode){  
...
//    uint16_t gain_setting = ads.getGain();
    ...
    if(adc_channel==0){
        ...
        Serial.print("Voltage Meas: read_voltage 2-3: ");
    }

    for (uint16_t i = 0; i < avg; i++){
        
        if(adc_channel==0){
            readout = (float)(adconverter->readADC_Differential_0_1());
        } else{
            readout = (float)(adconverter->readADC_Differential_2_3());
        }
    ...
    Serial.println("");
    Serial.print("Avg of readout[LSB]=");
    Serial.println(results/(float)avg);
    Serial.print("Avg of readout[microVolt]=");
    Serial.println(results/(float)avg * adc_gain_coeff * ATTENUATOR_COEFF);
    Serial.println("");
    
    // results = results / (float)avg * adc_gain_coeff * ATTENUATOR_COEFF;
 ...
 */
uint32_t Measurement::read_current(const uint16_t channel_23_offset){  // return measured current in [microAmp]
    ...
    for (uint16_t i = 0; i < avg; i++){
        readout = (float)(adconverter->readADC_Differential_2_3() - channel_23_offset);
        Serial.print(", "); Serial.print(readout);  
        results += readout;
    }
